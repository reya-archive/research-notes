...
3. 이미지 pull + 세 컨테이너 up (`openwebui-hosting-litellm`, `openwebui-hosting-tika`, `openwebui-hosting-webui`)
4. LiteLLM · Tika 헬스 체크 → Open WebUI 헬스 체크 (최대 각 2분)
...
docker compose ps
# 기대: openwebui-hosting-litellm · openwebui-hosting-tika · openwebui-hosting-webui
#       모두 Up · healthy

# 재시작 (예: litellm_config.yaml 수정 후 반영)
docker compose restart litellm

# Tika 단독 재시작 (JAVA_OPTS 조정 후)
...
Tika 는 호스트 포트를 공개하지 않아 호스트에서 `curl` 로는 확인할 수 없습니다. 컨테이너 안에서 호출합니다.

```bash
# Tika 컨테이너 자기 자신에서 (Tika 이미지에는 wget 기본 포함)
docker compose exec tika wget -q -O - http://127.0.0.1:9998/tika
# "This is Tika Server (Apache Tika 3.x.x). Please PUT ..." 응답이면 정상

# Open WebUI 컨테이너 → 내부 DNS 로 tika 해석 (실 사용 경로와 동일)
# 주의: open-webui 이미지에는 wget 이 없고 curl 이 들어 있습니다.
docker compose exec open-webui curl -s http://tika:9998/tika
```

후자가 동일한 Tika 메시지를 돌려주면 Open WebUI → Tika 경로 OK.
...
- 막혔다면 → [06-troubleshooting.md](./06-troubleshooting.md)
